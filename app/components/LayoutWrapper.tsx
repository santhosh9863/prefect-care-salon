...
function FloatingWhatsApp() {
  ...
  return (
    <div
      className={`fixed bottom-24 sm:bottom-6 right-4 sm:right-6 z-[60] transition-all duration-700 ease-out ${isVisible ? "opacity-100 translate-y-0 translate-x-0" : "opacity-0 translate-y-8 translate-x-4"
        }`}
    ...
    >
      ...
      >
        ...
        <span className={`absolute right-full mr-3 px-5 py-3 bg-white text-gray-800 text-sm font-medium rounded-2xl shadow-xl whitespace-nowrap transition-all duration-300 pointer-events-none ${isHovered ? "opacity-100 translate-x-0" : "opacity-0 translate-x-2"
          }`}>
...
function StickyMobileBooking() {
  ...
  return (
    <div
      className={`lg:hidden fixed bottom-0 left-0 right-0 z-[55] transition-all duration-500 ease-out ${isVisible ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"
        }`}
