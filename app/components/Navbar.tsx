...
function WhatsAppButton() {
  return (
    <a href="https://wa.me/919538111909" target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 z-40 flex items-center justify-center w-12 h-12 rounded-full bg-green-500 text-white shadow-lg hover:bg-green-600 transition-all" aria-label="WhatsApp">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-md shadow-sm" : "bg-white/90 backdrop-blur-md border-b border-gray-100"}`} style={{ height: '60px' }}>
        <nav className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
          ...
          <ul className="hidden lg:flex items-center gap-6">
            {navLinks.map((link) => (
              <li key={link.label}>
                <Link href={link.href} className="text-sm font-medium text-gray-600 hover:text-[#B91C1C] transition-colors">{link.label}</Link>
              </li>
            ))}
          </ul>
      ...
      {menuOpen && (
        <div ref={menuRef} className="lg:hidden fixed top-[60px] left-2 right-2 bg-white shadow-lg z-40 rounded-xl overflow-hidden">
          <ul className="py-2">
            {navLinks.map((link) => (
              <li key={link.label}>
                <Link href={link.href} onClick={() => setMenuOpen(false)} className="block px-4 py-3 text-gray-700 font-medium hover:text-[#B91C1C] hover:bg-gray-50">
                  {link.label}
                </Link>
              </li>
            ))}
            <li className="border-t border-gray-100 mt-2 pt-2">
              <Link href="/book" onClick={() => setMenuOpen(false)} className="block mx-3 py-3 bg-[#B91C1C] text-white text-center font-medium rounded-lg hover:bg-[#991B1B]">
                Book Appointment
              </Link>
            </li>
          </ul>
